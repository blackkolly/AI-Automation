...
// Service URLs
const services = {
  auth: process.env.AUTH_SERVICE_URL || 'http://auth-service.microservices.svc.cluster.local:3001',
  order: process.env.ORDER_SERVICE_URL || 'http://order-service.microservices.svc.cluster.local:3003',
  product: process.env.PRODUCT_SERVICE_URL || 'http://product-service.microservices.svc.cluster.local:8080'
};
...
app.get('/api/status', async (req, res) => {
  ...
  const overallHealth = Object.values(serviceStatus).every(service => service.status === 'healthy');
...
app.use('*', (req, res) => {
  res.status(404).json({
    ...
    availableRoutes: ['/api/auth', '/api/products', '/api/orders', '/health', '/api/status']
  });
});

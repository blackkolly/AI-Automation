...
build_and_push_images() {
    ...
    docker build -t "$DOCKER_REGISTRY/api-gateway:jaeger-v1.0" .
    ...
    docker build -t "$DOCKER_REGISTRY/order-service:jaeger-v1.0" .
    docker push "$DOCKER_REGISTRY/order-service:jaeger-v1.0"
    cd ../..
    
    # Auth Service (if it exists)
    if [ -d "services/auth-service" ]; then
        print_status "Building Auth Service..."
        cd services/auth-service
        docker build -t "$DOCKER_REGISTRY/auth-service:jaeger-v1.0" .
        docker push "$DOCKER_REGISTRY/auth-service:jaeger-v1.0"
        cd ../..
    else
        print_warning "Auth Service directory not found, skipping..."
    fi
...
update_image_tags() {
    print_status "Updating deployment image tags..."
    
    # Update API Gateway image
    kubectl set image deployment/api-gateway api-gateway="$DOCKER_REGISTRY/api-gateway:jaeger-v1.0" -n "$NAMESPACE"
    
    # Update Order Service image
    kubectl set image deployment/order-service order-service="$DOCKER_REGISTRY/order-service:jaeger-v1.0" -n "$NAMESPACE"
    
    # Update Auth Service image (if deployment exists)
    if kubectl get deployment auth-service -n "$NAMESPACE" >/dev/null 2>&1; then
        kubectl set image deployment/auth-service auth-service="$DOCKER_REGISTRY/auth-service:jaeger-v1.0" -n "$NAMESPACE"
    fi
    
    print_status "Image tags updated ✓"
}
